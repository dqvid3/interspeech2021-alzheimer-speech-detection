...
from src.dataset import ADDataset
...
if __name__ == "__main__":
    ...
    transcription_model_type = transcription_config['model_type']
    num_hypotheses = config.get('num_hypotheses')
    final_model_folder_name = f"{classifier_model_name}_hypo{num_hypotheses}_from_{transcription_model_type}"
    
    MODEL_DIR = os.path.join(RESULTS_BASE_DIR, final_model_folder_name)
    ...
    test_dataset = ADDataset(config['test_csv'], tokenizer, config['max_len'])
