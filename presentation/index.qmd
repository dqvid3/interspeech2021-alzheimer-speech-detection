...
subtitle: |
  ...
  <br><br>
  [ {{< fa brands github size=2xl >}} ](https://github.com/dqvid3/interspeech2021-alzheimer-speech-detection){.github-icon title="Vai al codice su GitHub" target="_blank"}
...
Secondo *Alzheimer’s Disease International*, i dati delineano un'emergenza sanitaria in rapida crescita.

::: columns
::: {.column width="50%" .fragment .fade-in-then-semi-out}
### Prevalenza in Esplosione
<div style="font-size: 0.9em;">
- **55+ milioni** di persone affette da demenza oggi nel mondo.
- **139 milioni** previsti entro il 2050.
- **L'Alzheimer è la causa primaria:** rappresenta il **60-70%** di tutti i casi
</div>
...
In Italia, paese con una delle popolazioni più anziane, la situazione è critica (Dati ISS/ISTAT).
...
### Costi e Famiglie
<div style="font-size: 0.9em;">
- **23 Miliardi €** di costo annuo stimato.
...
## Perché serve uno Screening Oggettivo?

Oltre ai numeri, ci sono barriere culturali e cliniche che l'IA può aiutare a superare.

::: columns
::: {.column width="50%" .fragment .fade-in-then-semi-out}
### Potenziale Preventivo
<div style="font-size: 0.9em;">
Circa il **45% dei casi** è legato a fattori modificabili (es. isolamento, ipoacusia, diabete). 
...
- **Qualità della Voce:** Variazioni di frequenza e ampiezza (Jitter/Shimmer).
</div>
...
- **Impoverimento Lessicale:** Ridotta diversità (*Type-Token Ratio* [@syed21_interspeech]) e uso di termini generici.
...
Il corpus è composto da registrazioni del task **"Cookie Theft"**.
...
3.  **Bias dell'Intervistatore:** Le registrazioni includono il clinico. Alcuni studi mostrano che classificare l'intervistatore (che adatta il linguaggio al paziente) può dare risultati migliori del paziente stesso, introducendo bias [@pereztoro21_interspeech].
...
**Velocità dell'elocuzione**<br>
I controlli **CN** parlano nettamente più veloce (**109** parole/min) rispetto ai pazienti **AD** (**85** parole/min).
</div>
...
I pazienti **AD** fanno pause più lunghe (**1.76s**) rispetto ai **CN** (**1.27s**), sintomo di difficoltà nel recupero lessicale.
</div>
...
*La quantità di informazione si riduce, ma la complessità lessicale rimane simile.*
...
<div style="font-size: 0.7em;">
**Conteggio Parole**<br>
I pazienti **AD** producono descrizioni più brevi (**112** parole) rispetto ai soggetti sani **CN** (**123** parole).
</div>
...
Differenza minima tra **AD (0.71)** e **CN (0.72)**. In questo stadio della malattia il vocabolario è ancora preservato.
</div>
...
*Quanto è "sicuro" il modello di trascrizione di aver capito le parole?*
...
<br>
*Il calo di confidenza nel gruppo AD indica che il parlato patologico è intrinsecamente più difficile da decodificare per l'IA*
</div>
...
<div style="font-size: 0.6em; text-align: center;">
*L'idea chiave è che l'acustica può compensare gli errori di trascrizione dell'ASR e viceversa.*
</div>
...
<div style="font-size: 0.6em; text-align: center;">
*Esporre il classificatore a diverse varianti di trascrizione (pesate dalla confidenza) lo rende intrinsecamente più **robusto** agli errori dell'ASR.*
</div>
...
if os.path.exists(AUDIO_ROOT) and os.path.isdir(AUDIO_ROOT):
  ...
  <button onclick="switchSubject('cn')" class="switch-btn active" id="btn-cn" style="background-color: #28a745;">🟢 Controllo Sano</button>
  <button onclick="switchSubject('ad')" class="switch-btn" id="btn-ad" style="background-color: #dc3545;">🔴 Paziente AD</button>
  </div>
  ...
  <div id="emoji-play" class="play-button paused">▶️</div>
  ...
  <script>
    ...
    function populateASRSelect(transcriptsDict) {{
      asrSelect.innerHTML = "";
      const keys = Object.keys(transcriptsDict).sort();
...
<div style="font-size: 0.9em;">
Come evidenziato da Pan et al., l'uso di ipotesi multiple funge da **Data Augmentation**. Esporre il classificatore a diverse varianti (anche errate o rumorose) della stessa frase lo rende più robusto alle incertezze dell'ASR.
</div>
...
La replicazione dello studio ha confermato la validità della pipeline proposta, con prestazioni che superano i benchmark originali.
...
- **Ruolo della Confidenza:** L'approccio *Confidence-Aware* si conferma il più performante. Tuttavia, l'uso di **BERT-Large** in questa specifica configurazione (rispetto al *Base* degli altri modelli) richiede cautela nell'attribuire il miglioramento esclusivamente allo score di confidenza.
:::
